...
// Export for testing - allows injecting a different "now"
export const getCurrentYear = () => new Date().getFullYear()

// Precise "now" as a decimal year (e.g. June 2026 ≈ 2026.41), so experience
// duration advances smoothly month-to-month instead of jumping once a year.
// Exported for testing.
export const getCurrentDecimalYear = () => {
  const now = new Date()
  const startOfYear = new Date(now.getFullYear(), 0, 1).getTime()
  const startOfNextYear = new Date(now.getFullYear() + 1, 0, 1).getTime()
  const fraction =
    (now.getTime() - startOfYear) / (startOfNextYear - startOfYear)
...
/**
 * Round a decimal years-elapsed figure to a display label like "4" or "4+".
 *
 * Rounds to the nearest whole year (so 3.9 -> 4, not 3). The "+" is only
 * appended when the precise value is past that rounded year (e.g. 4.2
 * rounds to 4 but is still shown as "4+" since there's progress beyond the
 * 4-year mark). An exact or just-under value (4.0, 3.9) shows no "+".
 * @internal Exported for testing
 */
export function roundYears(decimalYears: number): {
  rounded: number
  hasPlus: boolean
} {
  const rounded = Math.round(decimalYears)
  return { rounded, hasPlus: decimalYears > rounded }
}

export function roundYearsLabel(decimalYears: number): string {
  const { rounded, hasPlus } = roundYears(decimalYears)
  return `${rounded}${hasPlus ? '+' : ''}`
}

/**
 * Resolve the elapsed-years figure for a `careerStartYear`-style variable,
 * honoring the same whole-year vs decimal-year precision rules as
 * `processTemplateString`. Shared by both the template-string resolver and
 * the quickStats resolver below so the precision logic lives in one place.
 * @internal Exported for testing
 */
export function resolveYearsSinceVariable(
  varName: string,
  ...
  return Number.isInteger(startYear)
    ? currentYear - startYear
    : getCurrentDecimalYear() - startYear
}

/**
 * Process template strings like "{{yearsSince:careerStartYear}}"
 *
 * `variables` may hold whole years (e.g. 2022) or decimal years
 * (e.g. 2022.0 for January, 2022.5 for July) for month-precision results.
 * The years-elapsed figure is floored to whole years and evaluated against
 * the visitor's current date, so it advances automatically as time passes.
 *
 * A second token, "{{yearsSincePlus:varName}}", resolves to a rounded
 * label with a conditional "+" baked in (e.g. "4" or "4+") via
 * `roundYearsLabel`, for places where the "+" itself needs to depend on
 * the precise elapsed time rather than always being shown.
 * @internal Exported for testing
 */
export function processTemplateString(
  ...
  const withPlus = str.replace(
    /\{\{yearsSincePlus:(\w+)\}\}/g,
    (_match, varName) => {
      const elapsed = resolveYearsSinceVariable(varName, variables)
      return roundYearsLabel(elapsed)
    }
  )

  return withPlus.replace(/\{\{yearsSince:(\w+)\}\}/g, (_match, varName) => {
    const elapsed = resolveYearsSinceVariable(varName, variables)
    // Whole-year variables (e.g. 2022) preserve the original year-precision
    // behavior (an exact whole number). Decimal-year variables (e.g. 2022.0
    // for January) get month-precision, floored so "3.9 years" displays as
    // "3" until the anniversary month/day actually passes.
    return String(Number.isInteger(elapsed) ? elapsed : Math.floor(elapsed))
  })
}
...
const processedHero = {
  ...
  quickStats: rawContentConfig.hero.quickStats?.map((stat) => {
    if (typeof stat.value === 'string') {
      const yearsSinceMatch = stat.value.match(
        /^\{\{yearsSince(?:Plus)?:(\w+)\}\}$/
      )
      if (yearsSinceMatch) {
        const elapsed = resolveYearsSinceVariable(
          yearsSinceMatch[1],
          variables
        )
        const { rounded, hasPlus } = roundYears(elapsed)
        return {
          ...stat,
          value: rounded,
          suffix: hasPlus ? '+' : '',
...
/**
 * Years of professional experience, computed to month precision from the
 * configured career start. Re-evaluated fresh on every page load, so it
 * advances automatically as time passes rather than being baked in at build.
 * This is the floored whole-number figure (e.g. 3 for 3.9 years) — use
 * `yearsOfExperienceLabel` for the display string with proper rounding.
 */
export const yearsOfExperience = variables.careerStartYear
  ? Math.floor(getCurrentDecimalYear() - variables.careerStartYear)
  : 0

/**
 * Display label for years of experience, rounded to the nearest year with
 * a conditional "+" (e.g. 3.9 years -> "4", 4.2 years -> "4+"). This is
 * what should be shown to people; `yearsOfExperience` is the raw floored
 * count, kept for any logic that needs a plain whole number.
 */
export const yearsOfExperienceLabel = variables.careerStartYear
  ? roundYearsLabel(getCurrentDecimalYear() - variables.careerStartYear)
  : '0'
...
export const certifications: Certification[] =
  rawContentConfig.certifications ?? []

// Certifications section configuration
export const certificationsSection: CertificationsSection = {
  eyebrow: rawContentConfig.certificationsSection?.eyebrow ?? 'Credentials',
  headline: rawContentConfig.certificationsSection?.headline ?? 'Certifications',
  description: rawContentConfig.certificationsSection?.description,
}
...
export const experienceDescription =
  ...
  (yearsOfExperience > 0
    ? `${yearsOfExperienceLabel} years building and scaling cloud data platforms across enterprise teams.`
    : 'Building and scaling cloud data platforms across enterprise teams.')
