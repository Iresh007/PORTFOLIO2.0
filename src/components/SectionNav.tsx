      ...
      <div className="flex flex-col gap-2 bg-card/80 backdrop-blur-sm rounded-full p-2 shadow-lg border border-border">
        {navLinks.map((link) => {
          ...
          return (
            ...
            >
              {/* Tooltip */}
              <span className="absolute right-full mr-3 top-1/2 -translate-y-1/2 px-2 py-1 rounded bg-card dark:bg-white text-white dark:text-foreground text-xs font-medium whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity">
                {link.label}
              </span>
            </button>
          )
