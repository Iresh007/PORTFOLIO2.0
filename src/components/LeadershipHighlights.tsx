...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  'Pipeline Execution Time': Gauge,
  'Production Defects': ShieldCheck,
  'Schema Deployments': GitBranch,
  'ADF Pipelines Modernized': Workflow,
}
...
const categoryColors: Record<string, string> = {
  ...
  architecture: 'bg-navy-200 text-foreground dark:bg-muted dark:text-navy-300',
  leadership:
    'bg-slate-200 text-slate-800 dark:bg-slate-800 text-muted-foreground',
...
export function LeadershipHighlights() {
  ...
  return (
    <section id="impact" className="section-padding relative overflow-hidden" ref={pipelineRef}>
      <div className="pipeline-flow-line" aria-hidden="true" />
      <div className="container-wide">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16 pipeline-reveal">
          ...
          {metrics.map((metric, index) => {
            ...
            return (
              <FlipCard
                ...
                className={cn(
                  `h-[200px] md:h-[220px] cursor-pointer pipeline-reveal pipeline-reveal--delay-${Math.min(index + 1, 4)}`,
                ...
                front={
                  ...
                  >
                    <CardContent className="p-6 text-center relative h-full flex flex-col justify-center">
                      {/* Icon or Bauhaus Shape Badge, with progress ring as a halo behind it */}
                      {isBauhaus ? (
                        ...
                        <div className="relative w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                          {/* Progress ring halo, sized to just the icon badge */}
                          ...
                          <div className="relative inline-flex items-center justify-center w-12 h-12 rounded-xl bg-muted text-accent border border-border group-hover:bg-accent group-hover:text-accent-foreground transition-colors">
                      ...
                      {/* Value */}
                      <p className="relative text-3xl md:text-4xl font-mono font-bold text-card-foreground mb-1">
                        <AnimatedCounter
                          value={metric.value}
                    ...
                    {/* Hover accent */}
                    <div className="absolute bottom-0 left-0 right-0 h-1 bg-accent transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left" />
                  </Card>
                }
                back={
                  <Card className="group relative overflow-hidden border-border border-accent h-full bg-card">
                    <CardContent className="p-4 md:p-5 h-full flex flex-col">
                      ...
                      {/* Title */}
                      <h4 className="font-display font-semibold text-sm md:text-base text-foreground text-foreground mb-1 line-clamp-2">
                        {backContent.title}
                      </h4>
                      ...
                      {backContent.stats.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-3">
                          {backContent.stats.slice(0, 2).map((stat) => (
                            <div
                              key={stat.label}
                              className="text-center bg-slate-50 dark:bg-muted rounded px-2 py-1"
                            >
                              <p className="text-xs font-mono font-bold text-accent dark:text-accent">
                                {stat.value}
                              </p>
