...
/**
 * Country-specific label overrides — used when we want a more specific
 * label than the broad region name (e.g. "India" instead of "Asia-Pacific").
 */
...
/**
 * Get display label for a region. If a countryCode is provided and has a
 * more specific override (e.g. India within APAC), that is used instead.
 */
export function getRegionLabel(region: Region, countryCode?: string): string {
  if (countryCode) {
    const override = countryLabelOverrides[countryCode.toUpperCase()]
...
/**
 * Known India cities for on-site location detection
 */
...
/**
 * Map of region -> known on-site city set, used for region-aware on-site matching.
 * Only regions with curated city lists are included; others fall back to
 * generic text matching against the region label / country names elsewhere.
 */
const REGION_CITY_SETS: Partial<Record<Region, Set<string>>> = {
  EU: EU_CITIES,
  Americas: US_CITIES,
  APAC: INDIA_CITIES,
}
...
/**
 * Check if on-site locations are in the given region (region-aware)
 */
function hasOnSiteLocationInRegion(
  locations: string[],
  region: Region
): boolean {
  const citySet = REGION_CITY_SETS[region]
  if (!citySet) return false
  for (const loc of locations) {
    const locLower = loc.toLowerCase()
    if (citySet.has(locLower)) {
      return true
    }
  }
  return false
...
/**
 * Check if job matches "On-site (region)" filter
 * ON_SITE + HYBRID jobs in the user's region (defaults to EU for back-compat)
 */
export function matchesOnSiteEUFilter(
  ...
  // Check if on-site locations are in the user's region
  return hasOnSiteLocationInRegion(onSiteLocations, userRegion)
}

/**
 * Check if job matches "Any (region)" filter
 * All jobs available to the user's region: remote-global + remote-region + onsite-region
 */
export function matchesAnyEUFilter(
  ...
  // Check remote in region
  if (matchesRemoteEUFilter(locationData, userRegion)) {
    return true
  }

  // Check on-site in region
  if (matchesOnSiteEUFilter(locationData, userRegion)) {
    return true
  }

  // MIXED_ROLES: include if any part is available to the user's region
  if (locationData.type === 'MIXED_ROLES') {
    const { primaryRegions, excludedRegions, onSiteLocations } = locationData

    // Exclude if the region is explicitly excluded
    if (excludedRegions.includes(userRegion)) {
      return false
    }

    // Include if the region is primary
    if (primaryRegions.includes(userRegion)) {
      return true
    }

    // Include if has an on-site location in the region
    if (hasOnSiteLocationInRegion(onSiteLocations, userRegion)) {
      return true
    }

    // If has on-site locations but none in the region, exclude (clearly regional to those locations)
    if (onSiteLocations.length > 0) {
      return false
    }
...
export function matchesLocationFilter(
  ...
  switch (filter) {
    ...
    case 'onsite-eu':
      return matchesOnSiteEUFilter(locationData, userRegion)
    case 'any-eu':
